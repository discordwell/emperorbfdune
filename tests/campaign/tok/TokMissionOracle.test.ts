...
import {
  buildMissionOracleDataset,
  discoverScriptIds,
  readJsonFile,
  writeJsonFile,
} from './oracles/missionOracle';
import type { TokMissionOracleDatasetV1 } from './oracles/oracleTypes';

const TEST_DIR = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(TEST_DIR, '../../../');
const TOK_DIR = path.join(ROOT, 'assets/data/missions/tok');
const ORACLE_FILE = path.join(TEST_DIR, 'oracles', 'tok_mission_oracle.v1.json');
const DIFF_FILE = path.join(ROOT, 'artifacts', 'oracle-diffs', 'tok_mission_oracle.diff.json');
...
function buildSubsetDataset(expected: TokMissionOracleDatasetV1, scriptIds: string[]): TokMissionOracleDatasetV1 {
  return {
...
describe('Tok mission oracle', () => {
  it('matches deterministic mission checkpoint oracle (fast subset on PR, full on demand)', () => {
    const updateMode = process.env.TOK_UPDATE_ORACLES === '1';
    const fullMode = updateMode || process.env.TOK_ORACLE_FULL === '1';
    const allScripts = discoverScriptIds(TOK_DIR);

    if (updateMode) {
      const dataset = buildMissionOracleDataset({ scriptIds: allScripts });
      writeJsonFile(ORACLE_FILE, dataset);
    }
    ...
    const expected = readJsonFile<TokMissionOracleDatasetV1>(ORACLE_FILE);
    expect(expected.fastScripts.length).toBeGreaterThan(0);
    for (const scriptId of expected.fastScripts) {
      expect(expected.missions[scriptId]).toBeDefined();
    }
    ...
    const actual = buildMissionOracleDataset({
      scriptIds: selected,
      seed: expected.seed,
      defaultMaxTick: expected.defaultMaxTick,
      headerMaxTick: expected.headerMaxTick,
