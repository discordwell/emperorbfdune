...
### 2026-07-08T05:45UTC - Landed WIP (3 sim bugs) + Save/Load Determinism (Crates, Sandworm) + Harvester Capture + AoE Blast
- **8 genuine bugs fixed across 6 commits**, each with tests, each verified non-vacuous by path-scoped `git stash` of only the src file(s) (tests kept) → the fix-specific assertions fail against original source while controls pass. 4 parallel read-only bug-hunt agents (sandworm, save/load-completeness, combat/destruction, movement/harvest) seeded from prior deferred candidates + a final adversarial review agent on the whole session diff; every finding personally re-traced in source before fixing. Suite: **884 pass / 71 files** (was 867/69 at session start). Typecheck + production build clean throughout.
- **Landed the operator's in-progress WIP first** (3 sim bugs → 6 fails against original source): (a) in-flight delivery Carryalls (Health=9999 animation entities in unitQuery, tracked only by DeliverySystem) serialized as immortal phantom units on save/load → `DeliverySystem.getActiveCarryallEids()` + skip in buildSaveData; (b) `suppressedEntities` leaked on every building death (startDestruction suppresses combat but unregisterUnit ran first + ECS removal never touches CombatSystem) → clear suppression in removeEntity; (c) Starport orders stalled forever with no matching Factory/Barracks (queued in the factory/barracks queue → power/factory mult 0 → elapsed never advances; also wrongly halved by low power) → `QueueItem.starport` flag forces fixed 1.0/tick, round-trips via `{...i}` queue serialization. Commit ad48788.
- **Then extended the save/load-determinism theme** (same class as the already-fixed rngState + stormWaitTimer): the codebase deliberately round-trips `simRng.getState()` and the storm fix even avoids an extra draw to preserve it — but two non-ECS gated-RNG subsystems were still unsaved, so a save→load desynced the whole RNG stream (every later combat/AI/storm/worm draw shifts).
- **Crates** (commit ab46a10): `ctx.activeCrates` (Map) + `ctx.nextCrateId` are live gameplay state — GameTickHandler spawns a crate every 1000 ticks off simRng, gated by `activeCrates.size < 3`, and collection grants +CRATE_CREDITS/+CRATE_XP_BOOST/CRATE_HEAL_PCT. Absent from SaveData → crates vanished on load (lost payout) AND the size-gate flipped the next spawn draw. Plain Map, no ECS backing → must be serialized. Added `crates`/`nextCrateId` to SaveData; new `EffectsManager.clearAllCrates()`; restore clears+repopulates+respawns meshes (mirrors groundSplats). New `tests/core/SaveLoadCrates.test.ts` (6).
- **Sandworm subsystem** (commit c6844cf): `SandwormSystem` had NO save API — `worms[]` (pos/target/state/life + hunt-target & Fremen-rider eids), `tickCounter`, `thumpers[]`, per-side attract/repel flags all lost. Impact: worm hunting your harvester vanished; worm-rider stranded as a ground unit; **tickCounter→0 silenced ALL worm spawns for MIN_TICKS_WORM_CAN_APPEAR (1000t = 40s) after loading any save**; RNG desync via `worms.length < maxWorms` + `tickCounter % 100` spawn gate. Added `serialize()/deserialize()` (hunt/rider eids stored as save-array indices via the eid↔index maps like abilityState, remapped on load; unmappable rider → un-mount to roaming, unmappable hunt → null). Worm visuals auto-rebuild (updateWormVisuals is index-keyed off getWorms()). New `tests/core/SaveLoadSandworm.test.ts` (7: 6 serialize/deserialize unit + 1 end-to-end integration).
- **Harvester bound to a captured refinery** (commit 1d98f45): `isValidRefinery()` checked type/liveness/name but NOT owner — the lone sibling missing the owner filter that findRefinery + returnToRefinery's own reassignment loop both apply. Engineer capture flips `Owner.playerId[bid]` in place (AbilitySystem:563, no eid recycle, no event; buildings default canBeEngineered=true), so a captured refinery stayed "valid", returnToRefinery short-circuited, and the harvester kept shuttling into enemy territory forever, ignoring its own refineries. Fix: `owner` param on isValidRefinery (all 3 call sites have the harvester eid) → captured refinery invalid → retask to own refinery or IDLE. New `tests/simulation/harvest.test.ts` cases (retask, go-idle).
- **AoE blast retro-killed the survivors it spawned** (commit d710709): `applyBlastDamage` iterated the LIVE bitECS query (`healthQuery(world)`); when it dropped a building to 0 HP the synchronous `unit:died` → EventHandlers spawns survivor infantry inline (spawnUnit→addEntity) at the building centre (inside the blast) → appended to `healthQuery.dense` → hit by the still-running loop. So killing a production building with any AoE weapon silently voided the survivor mechanic (works fine for single-target kills). Fix: snapshot the query up front (`[...healthQuery(world)]`) — safe since all deaths defer removal (no mid-loop removals). New `tests/simulation/combat.test.ts` case drives applyBlastDamage with a unit:died→survivor-spawn listener.
- **Worm-rider command preserved on load** (commit dd47901, from the final adversarial review): restoreFromSave's snap-to-ground pass grounds+rallies elevated player-0 units; a restored mounted Fremen rider (live unit at y=1.5) got caught, so the re-linked worm chased the rally point instead of the player's command. Fix: run sandworm `deserialize` BEFORE the snap pass and skip current riders via new `SandwormSystem.getRiderEids()`. Low severity (mount survived, self-corrected) but completes the worm-restore properly. Review confirmed NO hard regressions elsewhere (ordering, symmetric eid remap, no aliasing, harvester owner-scope, AoE snapshot all verified correct).
- **Refuted leads** (agents, re-verified): `WORM_SPICE_DESTROY_RATE = 0.1` "vs 200" is NOT a bug — spice is normalized 0–1 per tile (SPICE_VALUE=200; harvester chunk 0.05, minimap `min(1,spice)`), and `RemoveSpice=200` in the comment is actually the **[HKBuzzsaw]** stat (rules.txt:8279), misattributed to the worm. Sandworm huntingEid/riderEid same-owner recycle: already guarded (UnitType+Health+Owner at :262-263), negligible (MAX_SURFACE_WORMS=1). Large-building eid-recycle "zombie" window: NOT real — getDyingEntities() has 0 callers, isDying() only used by a test, and bitECS recycle needs 1000 FIFO removals (unreachable in the ~219-tick window). New corpse-firing path: targets re-validated every tick.
- **Deferred (future pass)**: HarvestSystem internals not saved (harvestTimers/tickCounter/activeMound/bloomPending/noSpiceTimer/fleeing/airlifting — more entangled with ECS-derived harvester state that already round-trips); harvester `refEid: 0` hardcoded in save (self-heals via returnToRefinery nearest-refinery fallback + isValidRefinery `<=0` guard — sloppy but not sharp); minor: airlift landing uses `terrain.isPassable` (infantry passability) for a vehicle harvester; MovementSystem second arrival branch omits the stuckTicks/lastPos cleanup its sibling does; survivors with Combat appended to the main combatQuery loop can act the tick they spawn (broader than the blast fix).

### 2026-06-24T10:58UTC - Bugfixes: SW AI Idle, MCV Phantom Death, Credits Stat, Storm Save/Load, Hash Gaps
- **Five genuine bugs** found via 4 parallel read-only bug-hunt agents (save/load-determinism, combat/destruction, production/economy, abilities/superweapon/sandworm), then personally re-traced in source before fixing. Each fix verified non-vacuous by path-scoped `git stash` of *only* the src files (tests kept), re-running the new tests against original source and watching the fix-specific assertions fail (14 fails) while control/sanity tests still passed. Suite: **861 pass / 68 files** (was 843/65). Typecheck + production build clean.
- **AI superweapon never fired when the enemy had no buildings** (`SuperweaponSystem.ts`, confirmed deferred candidate). AI target selection scanned only `buildingQuery`; `bestCount` stays 0 when the human has units but zero structures (late-game razed base / units-only mission), so the charged Palace weapon sat full forever (the AI's only `fire()` call site). Fix: extracted a pure, DOM-free `static pickAiTarget(world, aiPid)` that picks the densest enemy-building cluster, else falls back to the densest **enemy-unit** cluster (skipping off-map passengers at x<-900), else null; `update()` calls it and applies the same RNG jitter. RNG draws are identical in the common (buildings-exist) case, so determinism is unchanged there. New `tests/simulation/superweapon.test.ts` (5).
- **AI MCV→ConYard deploy faked a unit death** (`EventHandlers.ts`). Consuming a produced MCV set `Health=0` and emitted plain `unit:died`, which (a) recorded a false `recordUnitLost` (MCV `countsForStats` defaults true) and (b) ran the full death pipeline — explosion + wreck + decal + minimap ping + a 'death' game event — at the AI base on every deploy. Fix: added `silent?: boolean` to the `unit:died` payload; when silent, stats/ping/VFX/anim and the event-queue 'death' push are skipped but cleanup (combat/movement/ability unregister) + deferred `removeEntity` still run. MCV consume now emits `silent: true`. New `tests/core/EventHandlers.test.ts` (silent vs normal control).
- **Credits-spent stat recorded base cost, not the amount actually deducted** (`EventHandlers.ts`). The `production:started` listener logged raw `def.cost`; `startProduction` deducts `getAdjustedCost` (`cost * difficulty multiplier`), so the post-game stat was wrong on every non-normal difficulty / for AI. Fix: record `productionSystem.getAdjustedCost(owner, unitType, isBuilding)`; the `>0` guard preserves "upgrades not counted". (Starport's separate variance pricing remains untracked — noted, out of scope.) Tests in `EventHandlers.test.ts`.
- **Sandstorm `stormWaitTimer` lost on save + RNG perturbed on load** (`SaveLoadSystem.ts` / `GameContext.ts`). `buildSaveData` never wrote it (it's a GameContext field, not previously in `SaveData`), and `restoreFromSave` *rerolled* it via `simRng.random()` — re-randomising storm cadence on every load AND consuming a draw, so a save→load diverged from an unsaved run. Fix: added `stormWaitTimer?` to `SaveData`, persist it, and restore it without any RNG draw (legacy saves without the field still reseed). New `tests/core/SaveLoadStormTimer.test.ts` asserts round-trip + `simRng.getState()` unchanged after restore.
- **SimulationHash blind to turret aim/cooldown/target** (`SimulationHash.ts`, latent — hash feeds only replay/desync which is built but unwired per memory). It hashed unit `Combat.fireTimer` but never `Rotation.y`/`TurretRotation.y`, and the building loop hashed **no** combat/targeting at all — so two states differing only in a turret's aim, fire timer, or target hashed equal (a desync blind spot, since aim gates firing in CombatSystem). Fix: hash `Rotation.y`/`TurretRotation.y` on units and `Combat.fireTimer`/`Rotation.y`/`TurretRotation.y`/`AttackTarget` on buildings (all `hasComponent`-guarded). Extended `tests/core/SimulationHash.test.ts` with 6 `computeSimulationHash` equality tests (first coverage of that fn).
- **Process note**: bug-hunt agents were strictly read-only (no source reverts this time). Agents surfaced larger deferred items intentionally left for a future pass: full Sandworm/Harvest *system* state (worms/riders/mound/tickCounters) not in save (HIGH, but needs eid-remapped serialization); uncollected crates not persisted; BuildingDestruction large-building eid-recycle "zombie building" window + `suppressedEntities` leak on recycle (both need 1000+ removals to trigger); sandworm stale `huntingEid` same-owner recycle; `WORM_SPICE_DESTROY_RATE` 0.1-vs-200 constant mismatch.
