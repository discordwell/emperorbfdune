...
- **7 genuine bugs fixed across 5 commits**, each with tests, each verified non-vacuous by path-scoped `git stash` of only the src file(s) (tests kept) → the fix-specific assertions fail against original source while controls pass. 4 parallel read-only bug-hunt agents (sandworm, save/load-completeness, combat/destruction, movement/harvest) seeded from prior deferred candidates; every finding personally re-traced in source before fixing. Suite: **883 pass / 71 files** (was 867/69 at session start). Typecheck + production build clean throughout.
